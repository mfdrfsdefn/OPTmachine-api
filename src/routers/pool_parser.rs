...
use crate::AppState;
use crate::dto::pool_parser::{PoolParserRequest, PoolAccountResponse};
use axum::{
    Router,
    extract::{Json, State},
    routing::post,
...
pub async fn pool_parser_service(
    ...
    let res = state.pool_parser_service.parse_pool_account(req).await.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(res))
}
