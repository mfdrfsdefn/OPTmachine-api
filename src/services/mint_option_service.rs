...
impl MintOptionService{
    ...
    pub async fn build_mint_option_tx(
        ...
        let minter_underlying_account_pubkey = get_associated_token_address(&minter_pubkey, &underlying_pubkey);
        let minter_quote_account_pubkey = get_associated_token_address(&minter_pubkey, &quote_pubkey);
        let minter_option_account_pubkey = get_associated_token_address(&minter_pubkey, &option_pubkey);
        let minter_underlying_account = solana_sdk::pubkey::Pubkey::new_from_array(minter_underlying_account_pubkey.to_bytes());
        let minter_quote_account = solana_sdk::pubkey::Pubkey::new_from_array(minter_quote_account_pubkey.to_bytes());
    ...
    Ok(MintOptionResponse { unsigned_tx: base64_tx, mint_amount: req.mint_amount})
    }
}