...
impl ExerciseOptionService{
    pub fn new(rpc_url: &str, program_id: Pubkey) -> Self {
        ...
        let args = ExerciseOptionArgs { exercise_amount: req.exercise_amount };
        ...
        let option_account_pubkey = mint_state.mint_authority.ok_or_else(|| anyhow::anyhow!("Mint has no authority"))?;
        let option_account_address = solana_sdk::pubkey::Pubkey::new_from_array(option_account_pubkey.to_bytes());
        ...
        let exerciser_option_account_ata = get_associated_token_address(&exerciser_pubkey, &option_mint_pubkey);
        let exerciser_quote_account_ata = get_associated_token_address(&exerciser_pubkey, &quote_mint_pubkey);
        let exerciser_underlying_account_ata = get_associated_token_address(&exerciser_pubkey, &underlying_mint_pubkey);
        let vault_ata = get_associated_token_address(&option_account_pubkey, &underlying_mint_pubkey);
        let creator_quote_account_ata = get_associated_token_address(&creator_pubkey, &quote_mint_pubkey);
        let exerciser_option_account = Pubkey::new_from_array(get_associated_token_address(&exerciser_pubkey, &option_mint_pubkey).to_bytes());
        let exerciser_quote_account = Pubkey::new_from_array(get_associated_token_address(&exerciser_pubkey, &quote_mint_pubkey).to_bytes());
        let exerciser_underlying_account = Pubkey::new_from_array(get_associated_token_address(&exerciser_pubkey, &underlying_mint_pubkey).to_bytes());
        let vault = Pubkey::new_from_array(get_associated_token_address(&option_account_pubkey, &underlying_mint_pubkey).to_bytes());
        let creator_quote_account = Pubkey::new_from_array(get_associated_token_address(&creator_pubkey, &quote_mint_pubkey).to_bytes());
    ...
    Ok(ExerciseOptionResponse { unsigned_tx: base64_tx, exercise_amount: req.exercise_amount})
    }
}